...
def load_dataset(file_path: Path) -> pd.DataFrame:
    ...
    print("=" * 80)
    return df
# ==================== Simple Transformer模型 ====================
class SimpleTransformer(nn.Module):
    def __init__(self, num_features: int, num_outputs: int, d_model: int = 64, nhead: int = 4, num_layers: int = 4, dim_feedforward: int = 256, dropout: float = 0.1) -> None:
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, dropout=dropout, batch_first=True, activation="relu", norm_first=False)
...
def calculate_component_metrics(y_true: np.ndarray, y_pred: np.ndarray, fold_number: int) -> list[dict]:
    ...
    return records
# ==================== 五折KFold ====================
def run_five_fold_cv(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if y_all.ndim != 2 or y_all.shape[1] != len(TARGET_COLUMNS):
        raise ValueError(f"目标矩阵形状错误：{y_all.shape}")
    ...
    print()
    print("=" * 80)
    print("Simple Transformer cross-validation: shuffled five-fold KFold")
    ...
    for fold_number, (outer_train_index, outer_test_index) in enumerate(splitter.split(X_all), start=1):
        print()
        print("=" * 80)
        print(f"Simple Transformer Fold {fold_number}/{N_SPLITS}")
        print("=" * 80)
        ...
        model = SimpleTransformer(num_features=len(FEATURE_COLUMNS), num_outputs=len(TARGET_COLUMNS), d_model=D_MODEL, nhead=N_HEAD, num_layers=N_LAYERS, dim_feedforward=DIM_FEEDFORWARD, dropout=DROPOUT).to(DEVICE)
        ...
        if y_pred.shape != y_test.shape:
            raise RuntimeError(f"预测矩阵形状错误，预期{y_test.shape}，实际{y_pred.shape}")
        ...
        checkpoint = {"model_state_dict": model.state_dict(), "feature_columns": FEATURE_COLUMNS, "target_columns": TARGET_COLUMNS, "d_model": D_MODEL, "nhead": N_HEAD, "num_layers": N_LAYERS, "dim_feedforward": DIM_FEEDFORWARD, "dropout": DROPOUT, "fold": fold_number, "scaler_X_mean": scaler_X.mean_, "scaler_X_scale": scaler_X.scale_, "scaler_y_mean": scaler_y.mean_, "scaler_y_scale": scaler_y.scale_}
        torch.save(checkpoint, f"best_simple_transformer_fold_{fold_number}.pth")
        del model
        if torch.cuda.is_available():
    ...
    return metrics_df, summary_df, oof_df, history_df
# ==================== OOF总体指标 ====================
...
def plot_oof_true_vs_pred(oof_df: pd.DataFrame) -> None:
    for target_name in TARGET_COLUMNS:
        ...
        plt.title(f"Simple Transformer Five-fold OOF Prediction for {target_name}\n$R^2$ = {r2:.4f}", fontsize=17)
        plt.legend(fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        output_path = Path(f"simple_transformer_cv_oof_{target_name}.png")
...
def plot_fold_metrics(metrics_df: pd.DataFrame) -> None:
    for target_name in TARGET_COLUMNS:
        ...
        plt.title(f"Simple Transformer Five-fold Error Metrics for {target_name}", fontsize=17)
        plt.xticks(range(1, N_SPLITS + 1))
        plt.legend(fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        output_path = Path(f"simple_transformer_cv_fold_metrics_{target_name}.png")
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.show()
        plt.close()
        print(f"Saved plot: {output_path.resolve()}")
...
def plot_training_history(history_df: pd.DataFrame) -> None:
    for fold_number in range(1, N_SPLITS + 1):
        ...
        plt.plot(fold_history["Epoch"], fold_history["Validation_Loss"], linewidth=2, label="Validation loss")
        plt.xlabel("Epoch", fontsize=16)
        plt.ylabel("Standardized MSE loss", fontsize=16)
        plt.title(f"Simple Transformer Training History - Fold {fold_number}", fontsize=17)
        plt.legend(fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        output_path = Path(f"simple_transformer_training_history_fold_{fold_number}.png")
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.show()
        plt.close()
        print(f"Saved plot: {output_path.resolve()}")
...
def print_cv_summary(summary_df: pd.DataFrame) -> None:
    print()
    print("=" * 80)
    print("Simple Transformer five-fold cross-validation summary")
    print("=" * 80)
...
def main() -> None:
    ...
    metrics_df, summary_df, oof_df, history_df = run_five_fold_cv(df)
    oof_metrics_df = calculate_oof_metrics(oof_df)
    metrics_df.to_csv("simple_transformer_cv_metrics_each_fold.csv", index=False, encoding="utf-8-sig", float_format="%.10g")
    summary_df.to_csv("simple_transformer_cv_metrics_summary.csv", index=False, encoding="utf-8-sig", float_format="%.10g")
    oof_df.to_csv("simple_transformer_cv_oof_predictions.csv", index=False, encoding="utf-8-sig", float_format="%.10g")
    history_df.to_csv("simple_transformer_cv_training_history.csv", index=False, encoding="utf-8-sig", float_format="%.10g")
    oof_metrics_df.to_csv("simple_transformer_cv_oof_overall_metrics.csv", index=False, encoding="utf-8-sig", float_format="%.10g")
    print_cv_summary(summary_df)
    print()
    print("=" * 80)
    print("Simple Transformer overall out-of-fold metrics")
    print("=" * 80)
    ...
    print("Output files")
    print("=" * 80)
    print("simple_transformer_cv_metrics_each_fold.csv")
    print("simple_transformer_cv_metrics_summary.csv")
    print("simple_transformer_cv_oof_predictions.csv")
    print("simple_transformer_cv_oof_overall_metrics.csv")
    print("simple_transformer_cv_training_history.csv")
    print("best_simple_transformer_fold_1.pth ... best_simple_transformer_fold_5.pth")
    print("simple_transformer_cv_oof_CL.png")
    print("simple_transformer_cv_oof_Cm.png")
    print("simple_transformer_cv_fold_metrics_CL.png")
    print("simple_transformer_cv_fold_metrics_Cm.png")
    print("simple_transformer_training_history_fold_1.png ... fold_5.png")
    plot_oof_true_vs_pred(oof_df)
    plot_fold_metrics(metrics_df)
    plot_training_history(history_df)
    print()
    print("All Simple Transformer five-fold comparison tasks completed.")
