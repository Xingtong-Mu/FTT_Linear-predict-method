...
# ==================== 数值型 TabTransformer 模型 ====================
class NumericalTabTransformer(nn.Module):
    """
    数值型 TabTransformer。

    说明：
    原始 TabTransformer 主要用于类别特征建模。
    由于当前气动力数据只有连续变量 MA、BETA、AA，
    因此这里将每个连续变量映射为一个特征 token，
    再通过 Transformer 编码器学习特征间关系。
    """
    ...
    def forward(self, x):
        ...
        # 连续特征 token 化
        # (batch, num_features, 1) * (num_features, d_model)
        x = x.unsqueeze(-1)
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, epochs=epochs):
    ...
    best_model_path = 'best_tabtransformer.pth'
...
def plot_true_vs_pred(
    ...
    target_names,
    component_metrics,
    save_path='tabtransformer_comparison.png'
):
    n = len(target_names)
